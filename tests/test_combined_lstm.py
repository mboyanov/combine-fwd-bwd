...
def test_should_combine_embeddings(fwd, bwd):
    ...
    # THEN
    np.testing.assert_array_equal(combined_embedding.detach().numpy()[:, :embedding_size], fwd.encoder(inp).detach().numpy())
    np.testing.assert_array_equal(combined_embedding.detach().numpy()[:, embedding_size:], bwd.encoder(inp).detach().numpy())